...
void ensure_directory_exist(fs::path const& dir)
{
    if (!fs::exists(dir))
    {
#ifdef USE_BOOST_FILESYSTEM
    ...
    std::function<void(std::string_view)> reporter)
{
    ...
    for (size_t i = 0; i < zipArchive->GetEntriesCount(); ++i)
    {
        ...
        if (entry)
        {
            ...
            {
                ensure_directory_exist(filepath);
            }
            else
            {
                ensure_directory_exist(filepath.parent_path());
...
int main()
{
    ...
    if (option == 'c')
    {
        ...
        std::string password;
        std::cout << "Enter file or dir to compress:"; //압축할 파일이 존재하는 경로
        std::cin >> inputpath;
        std::cout << "Enter archive path:"; // 압축후 파일 이름
        std::cin >> archivepath;
        std::cout << "Enter password:";
        std::cin >> password;

        compress_p(inputpath, archivepath, password, [](std::string_view message) {std::cout << message << std::endl; });
    }
    else if (option == 'd')
    {
        std::string archivepath;
        ...
        std::string password;
        std::cout << "Enter dir to decompress:";//압축해제할 경로
        std::cin >> outputpath;
        std::cout << "Enter archive path:";// 압축파일 이름
        std::cin >> archivepath;
        std::cout << "Enter password:";
        std::cin >> password;

        decompress_p(outputpath, archivepath, password, [](std::string_view message) {std::cout << message << std::endl; });
    }
    else
    {
